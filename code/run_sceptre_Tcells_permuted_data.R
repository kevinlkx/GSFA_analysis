...
## Run SCEPTRE analysis for T cells data
...
parser <- add_option(parser,"--condition",type="character",default="stimulated")
...
if(condition == "stimulated" || condition == "S"){
  selected_cells <- rownames(metadata)[which(endsWith(metadata$orig.ident, "S"))]
  cat(length(selected_cells), "stimulated cells. \n")
}else if(condition == "unstimulated" || condition == "n"){
  selected_cells <- rownames(metadata)[which(endsWith(metadata$orig.ident, "N"))]
  cat(length(selected_cells), "unstimulated cells. \n")
}else{
  stop("condition needs to be stimulated or unstimulated!")
}

# gene expression matrix (gene x cell)
gene_matrix <- TCells_data@assays$RNA@counts[, selected_cells]
gene_matrix[1:5, 1:3]
dim(gene_matrix)

# covariate matrix
covariate_matrix <- metadata[selected_cells, c('nCount_RNA', 'nFeature_RNA', 'percent_mt', 'gRNA_umi_count')]
...
combined_perturbation_matrix <- t(metadata[selected_cells, 4:24])
dim(combined_perturbation_matrix)
rownames(combined_perturbation_matrix)
...
if (!permute){
  ...
  set.seed(seed)
  outdir <- paste0(outdir, "/", condition)
  ...
  outdir <- paste0(outdir, "/", condition, "/", "perm_", perm_num)
  if(!dir.exists(outdir)) dir.create(outdir, recursive = TRUE)
}
