...
# pdf(file.path(res_dir, "LUHMES_gsfa_mast_degs_venn_diagram.pdf"))
...
# pdf(file.path(res_dir, "LUHMES_gsfa_mast_degs_qqplot_v2.pdf"))
qqplot.pvalue(pvalue_list, pointSize = 1, legendSize = 4) + 
  ggtitle("") + theme(plot.title = element_text(hjust = 0.5)) +
  scale_colour_discrete(name="Method")
# dev.off()
...
# pdf(file.path(res_dir, "Tcells_gsfa_mast_degs_qqplot_v2.pdf"))
qqplot.pvalue(pvalue_list, pointSize = 1, legendSize = 4) + 
  ggtitle("") + theme(plot.title = element_text(hjust = 0.5)) +
...
<!-- length(gsfa_only_degs) -->
<!-- length(mast_only_degs) -->
...
<!-- mast_res[which(mast_res$mast_only_gene==1), ] %>% arrange(PValue) %>% head() -->

<!-- mast_res[which(mast_res$gsfa_only_gene==1), ] %>% arrange(PValue) %>% head() -->

<!-- pvalue_list <- list('GSFA'=dplyr::filter(mast_res,gsfa_gene==1)$PValue, -->
<!--                     'GSFA [not MAST]'=dplyr::filter(mast_res,gsfa_only_gene==1)$PValue, -->
<!--                     'MAST [not GSFA]'=dplyr::filter(mast_res,mast_only_gene==1)$PValue, -->
<!--                     'MAST all tested genes'=mast_res$PValue) -->

<!-- pdf(paste0(res_dir, "/LUHMES_", m, "_gsfa_mast_degs_qqplot_v2.pdf")) -->
<!-- qqplot.pvalue(pvalue_list, pointSize = 1, legendSize = 4) +  -->
...
# pdf(file.path(res_dir, "Tcells_gsfa_mast_degs_venn_diagram.pdf"))
ggvenn(deg_list, fill_color = c("#00BA38","#619CFF"), show_percentage = FALSE, set_name_size = 6, text_size = 6)
# dev.off()
```
