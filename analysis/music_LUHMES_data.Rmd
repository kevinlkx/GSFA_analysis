...
* [R script](https://github.com/kevinlkx/GSFA_analysis/tree/main/code/run_music_LUHMES.R).
* [sbatch script](https://github.com/kevinlkx/GSFA_analysis/tree/main/code/run_music_LUHMES_data.sbatch).
...
summary_df <- summary_df %>%
  mutate(empirical_p_val_adj = p.adjust(empirical_pval, method = "BH"))
...
fdr_mat <- dcast(summary_df %>% dplyr::select(knockout, topic, empirical_p_val_adj), knockout ~ topic)
...
```{r effect_fdr_dotplot_20topics, eval=TRUE, fig.width=9, fig.height=5.5}
...
```{r effect_fdr_dotplot_4topics, eval=TRUE, fig.width=4, fig.height=5.5}
...
<!-- optimalModel <- readRDS("music_output/optimalModel_4_topics.rds") -->
...
<!-- model <- optimalModel -->
<!-- my_beta <- exp(model@beta) -->
<!-- topic_num <- nrow(my_beta) -->
<!-- my_geneName <- model@terms -->
<!-- colnames(my_beta) <- my_geneName -->
<!-- rownames(my_beta) <- paste0('Topic_',1:nrow(my_beta)) -->

<!-- my_beta[3, ] -->

<!-- head(sort(my_beta[3, ], decreasing = T)) -->

<!-- head(my_beta[3, ][order(my_beta[3, ], decreasing=T)]) -->

<!-- length(perturb_information) -->
<!-- table(perturb_information) -->

<!-- target_cells_idx <- which(perturb_information == "ADNP") -->
<!-- ctrl_cells_idx <- which(perturb_information == "CTRL") -->

<!-- head(expression_profile[, target_cells_idx]) -->

<!-- head(expression_profile[, ctrl_cells_idx]) -->
<!-- ``` -->
