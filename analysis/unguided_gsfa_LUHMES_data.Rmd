...
* R script: `/home/kaixuan/projects/GSFA_analysis/code/run_unguided_gsfa_LUHMES.R`  
* sbatch script: `/home/kaixuan/projects/GSFA_analysis/code/run_unguided_gsfa_LUHMES.sbatch`
...
dotplot_effectsize(t(stat_mat), t(fdr_mat),
                   reorder_markers = c(KO_names[KO_names!="Nontargeting"], "Nontargeting"),
                   color_lgd_title = "Linear regression\nT-statistics",
                   size_lgd_title = "FDR",
                   max_score = 4,
                   min_score = -4,
...
dotplot_effectsize(t(stat_mat), t(bonferroni_mat),
                   reorder_markers = c(KO_names[KO_names!="Nontargeting"], "Nontargeting"),
                   color_lgd_title = "Linear regression\nT-statistics",
...
DEGs.factors <- foreach(i=1:ncol(F_pm)) %do% {
  rownames(F_pm)[which(F_pm[,i]>0.95)]
}
names(DEGs.factors) <- colnames(F_pm)

associated.factors <- foreach(i=1:nrow(fdr.mat)) %do% which(fdr.mat[i,]<0.05)

guide.DEGs <- foreach(i=1:nrow(fdr.mat)) %do% unique(unlist(DEGs.factors[ associated.factors[[i]] ]))
names(guide.DEGs) <- rownames(fdr.mat)

num.guide.DEGs <- sapply(guide.DEGs, length)
num.guide.DEGs
```
...
# pdf(file.path(res_dir, "LUHMES_mast_pval_qqplots.pdf"),width = 12,height = 12)
...
# pdf(file.path(res_dir, "LUHMES_mast_pval_qqplots_gsfa_only_vs_unguided_gsfa_only.pdf"),width = 12,height = 12)
grid.arrange(grobs = qqplots, nrow = 4, ncol = 2)
# dev.off()
```
