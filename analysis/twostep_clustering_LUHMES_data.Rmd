...
Regress out total UMI counts per cell and percent of mitochondrial genes detected per cell and scaled to obtain gene level z-scores. The results of this are stored in combined_obj[["RNA"]]@scale.data
...
Embed cells in K-nearest neighbor (KNN) graph using `FindNeighbors()` using the first 30 PCs. 
Then apply the Louvain algorithm to find clusters using `FindClusters() function` with default resolution (0.8).
