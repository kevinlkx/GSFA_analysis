...
slurm setting
```{bash eval=FALSE}
sinteractive --partition=broadwl --account=pi-xinhe --mem=50G --time=10:00:00 --cpus-per-task=10
sinteractive --partition=xinhe --account=pi-xinhe --mem=50G --time=10:00:00 --cpus-per-task=10
```
...
summary_df <- summary_df %>%
  mutate(topic = factor(topic, levels = paste0("Topic_", 20:1)))
...
```{r effect_fdr_dotplot_20topics, eval=TRUE, fig.width=12, fig.height=8}
...
dotplot_effectsize(t(effect_mat), t(fdr_mat),
                   ...
                   color_lgd_title = "MUSIC empirical T statistics",
                   size_lgd_title = "FDR",
                   max_score = 20,
                   min_score = -20,
                   by_score = 10)
...
summary_df <- summary_df %>%
  mutate(topic = factor(topic, levels = paste0("Topic_", 4:1)))
...
```{r effect_fdr_dotplot_4topics, eval=TRUE, fig.width=12, fig.height=8}
